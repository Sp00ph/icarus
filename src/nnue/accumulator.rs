...
pub fn acc_add(acc: &mut [i16; L1], weights: &[[i16; L1]; INPUT], add: usize) {
    for (acc, weight) in acc.iter_mut().zip(&weights[add]) {
        *acc += *weight;
    }
}

pub fn acc_sub(acc: &mut [i16; L1], weights: &[[i16; L1]; INPUT], sub: usize) {
...
pub fn acc_add2_sub2(
    ...
    let add1: &[i16; L1] = &weights[add1];
    let add2: &[i16; L1] = &weights[add2];
    let sub1: &[i16; L1] = &weights[sub1];
    let sub2: &[i16; L1] = &weights[sub2];
