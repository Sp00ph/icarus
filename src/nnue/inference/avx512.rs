...
pub fn forward(us: &[i16; HL], them: &[i16; HL], bucket: usize) -> i32 {
    unsafe { forward_impl(us, them, bucket) }
}

#[target_feature(enable = "avx512bw")]
fn forward_impl(us: &[i16; HL], them: &[i16; HL], bucket: usize) -> i32 {
    const { assert!(HL.is_multiple_of(128)) };
    assert!(bucket < OUT_BUCKETS);
    ...
    let us_weights = NET.out_weight[bucket][0].as_ptr().cast::<__m512i>();
    let them_weights = NET.out_weight[bucket][1].as_ptr().cast::<__m512i>();
    ...
    for i in 0..HL / 128 {
        let us0 = unsafe { _mm512_loadu_si512(us_ptr.add(4 * i + 0)) };
        let us1 = unsafe { _mm512_loadu_si512(us_ptr.add(4 * i + 1)) };
        let us2 = unsafe { _mm512_loadu_si512(us_ptr.add(4 * i + 2)) };
        let us3 = unsafe { _mm512_loadu_si512(us_ptr.add(4 * i + 3)) };
        ...
        let us_weights0 = unsafe { _mm512_loadu_si512(us_weights.add(4 * i + 0)) };
        let us_weights1 = unsafe { _mm512_loadu_si512(us_weights.add(4 * i + 1)) };
        let us_weights2 = unsafe { _mm512_loadu_si512(us_weights.add(4 * i + 2)) };
        let us_weights3 = unsafe { _mm512_loadu_si512(us_weights.add(4 * i + 3)) };

        let them0 = unsafe { _mm512_loadu_si512(them_ptr.add(4 * i + 0)) };
        let them1 = unsafe { _mm512_loadu_si512(them_ptr.add(4 * i + 1)) };
        let them2 = unsafe { _mm512_loadu_si512(them_ptr.add(4 * i + 2)) };
        let them3 = unsafe { _mm512_loadu_si512(them_ptr.add(4 * i + 3)) };
        ...
        let them_weights0 = unsafe { _mm512_loadu_si512(them_weights.add(4 * i + 0)) };
        let them_weights1 = unsafe { _mm512_loadu_si512(them_weights.add(4 * i + 1)) };
        let them_weights2 = unsafe { _mm512_loadu_si512(them_weights.add(4 * i + 2)) };
        let them_weights3 = unsafe { _mm512_loadu_si512(them_weights.add(4 * i + 3)) };
